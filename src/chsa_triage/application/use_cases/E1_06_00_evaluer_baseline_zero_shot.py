...
from chsa_triage.domain.ports.formateur_invite_zero_shot import FormateurInviteZeroShot
...
def _extraire_texte_reponse_reelle(exemple: ExemplePivot) -> str:
    """`completion` est un tuple de `Message` (assistant seul en pratique) ; concatene leur `contenu`."""
...
class EvaluerBaselineZeroShotUseCase:
    ...
    def executer(self) -> ResultatEvaluationBaseline:
        ...
        """
        exemples = list(
            self.repository.lister(filtre={"split": TypeSplit.TEST_CLINIQUE, "type_exemple": TypeExemple.SFT})
        )
        if not exemples:
            raise ValueError(
                "aucun exemple type_exemple=SFT avec split=test dans le repository : rien a evaluer"
            )

        self.suivi.demarrer_run(self.nom_run, {"nombre_exemples": len(exemples), **self.parametres_generation})

        paires: list[tuple[str, str]] = []
        latences_ms: list[float] = []
        for exemple in exemples:
            invite = self.formateur.formater_invite_zero_shot(exemple)
            parametres = {"invite_deja_rendue": True, **self.parametres_generation}
            reponse = self.moteur.generer([{"role": "user", "content": invite}], parametres)
            paires.append((reponse.texte, _extraire_texte_reponse_reelle(exemple)))
        ...
        if resultat.exactitude_niveau.exactitude is not None:
            self.suivi.logger_metrique("exactitude_niveau_triage", resultat.exactitude_niveau.exactitude, 0)
        self.suivi.logger_metrique("latence_ms_moyenne", resultat.latence_ms_moyenne, 0)
