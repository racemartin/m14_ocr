"""
Point d'entree CLI, Etape 1, action "extraire le sous-ensemble DPO
destine a la publication" (§9 du README, cahier des charges §7
Livrable 1 : "SFT ~5000 paires + DPO").

Meme patron exact que `E1_05_02_extraire_sous_ensemble_sft.py` : filtre
le pivot ANONYMISE (`--dataset`) sur les exemples `TypeExemple.DPO`
deja repartis en split (`split is not None`, cf.
`E1_05_00_decouper_splits.py`), retire ceux listes dans `--exclusions`
(produit par `E1_04_01_reviser_pii_residuelle.py exporter`, cf. §6 et
§9 du README : identifiants portant un candidat VERDICT_CONFIRME ou
...
log = LogTool(origin="extraire_sous_ensemble_dpo")
...
def chemin_sortie_defaut(taille: int) -> str:
    return f"data/processed/dataset_chsa_triage_dpo_anonymise_{taille}.jsonl"
...
def main() -> None:
    ...
    parser.add_argument(
        "--sortie",
        default=None,
        help="Chemin de sortie (defaut : data/processed/dataset_chsa_triage_dpo_anonymise_<taille>.jsonl)",
    )
    arguments = parser.parse_args()
    if arguments.sortie is None:
        arguments.sortie = chemin_sortie_defaut(arguments.taille)

    log.START_ACTION("extraire_sous_ensemble_dpo", "main", "extraction du sous-ensemble DPO a publier")
    ...
    log.FINISH_ACTION(
        "extraire_sous_ensemble_dpo", "main", f"{len(resultat)} exemple(s) ecrits dans {arguments.sortie}"
    )
