...
def main() -> None:
    ...
    parser.add_argument("--dataset", default=CHEMIN_DATASET_DEFAUT, help="Chemin du pivot ANONYMISE (champ split renseigne)")
    parser.add_argument("--url-serveur", default=URL_SERVEUR_DEFAUT, help="URL d'un llama-server DEJA LANCE")
    parser.add_argument("--modele", default=MODELE_DEFAUT, help="Nom HF du modele (tokenizer/chat template reel)")
    parser.add_argument("--temperature", type=float, default=0.0, help="0.0 = generation deterministe (reproductible)")
    parser.add_argument("--n-predict", type=int, default=256, help="Nombre max de tokens generes par exemple")
    parser.add_argument("--timeout-secondes", type=float, default=180.0, help="Timeout HTTP par requete d'inference")
    parser.add_argument("--suivi-uri", default=URI_SUIVI_MLFLOW_DEFAUT, help="URI MLflow (meme defaut que training/E2_04_sft_train.py)")
    parser.add_argument("--nom-run", default=NOM_RUN_DEFAUT)
    arguments = parser.parse_args()

    log.START_ACTION("evaluer_baseline", "main", "evaluation baseline zero-shot (Etape 1bis)")
    ...
    moteur = LlamaCppInferenceAdapter(url_serveur_local=arguments.url_serveur, timeout_secondes=arguments.timeout_secondes)
    suivi = MlflowSuiviExperimentation(uri_tracking=arguments.suivi_uri)

    # -------------------------------------------------------------------------
    # USE CASE EXECUTE
    # -------------------------------------------------------------------------
    log.STEP(1, "Generation zero-shot + comparaison sur le split test (peut prendre du temps sur CPU)")
    try:
        cas_usage = EvaluerBaselineZeroShotUseCase(
            ...
            moteur=moteur,
            suivi=suivi,
            parametres_generation={"temperature": arguments.temperature, "n_predict": arguments.n_predict},
            nom_run=arguments.nom_run,
        )
        resultat = cas_usage.executer()
    except Exception as erreur:
        log.LEVEL_4_ERROR("evaluer_baseline", f"echec de l'evaluation baseline : {erreur}")
        raise
    ...
    log.FINISH_ACTION("evaluer_baseline", "main", f"run '{arguments.nom_run}' journalise dans {arguments.suivi_uri}")

    print(f"Exemples evalues (split=test, type_exemple=SFT) : {resultat.nombre_exemples}")
    print(f"Exact match : {resultat.exact_match:.3f}")
    print(f"F1 moyen (token) : {resultat.f1_moyen:.3f}")
    if resultat.exactitude_niveau.exactitude is not None:
        print(
            f"Exactitude classification niveau ESI : {resultat.exactitude_niveau.exactitude:.3f} "
            f"({resultat.exactitude_niveau.nombre_comparables}/{resultat.exactitude_niveau.nombre_paires} paires comparables)"
        )
    else:
        print(
            "Exactitude classification niveau ESI : non calculable (0 paire comparable sur "
            f"{resultat.exactitude_niveau.nombre_paires} ; le dataset actuel n'a pas de completion au format "
            "JSON {niveau, categorie, ressources_estimees}, cf. docs/03_etape2_sft/00_introduction_concepts.md)"
        )
    print(f"Latence moyenne par generation : {resultat.latence_ms_moyenne:.1f} ms")
    print(f"Run MLflow '{arguments.nom_run}' journalise dans {arguments.suivi_uri}")
